...
conn=mysql.connector.connect( host='localhost',user='root',password='',database='user',port='3306',consume_results=True)
...
@flask_app.route('/breastinfo')
def breastinfo():
    return render_template("breastinfo.html")


@flask_app.route('/lunginfo')
def lunginfo():
    return render_template("lunginfo.html")


@flask_app.route('/liverinfo')
def liverinfo():
    return render_template("liverinfo.html")


@flask_app.route('/cervicalinfo')
def cervicalinfo():
    return render_template("cervicalinfo.html")
#################################################################################################
...
def getstarted():
    ...
    cursor.execute("""SELECT `BREAST CANCER`, `CERVICAL CANCER`, `LUNG CANCER`, `LIVER CANCER` FROM `patients` WHERE id = %s""", (user_id,))
    patient_data = cursor.fetchone()
    
    print(patient_data)
    new_patient_data = []
    for i in range(len(patient_data)):
        if patient_data[i] in ('NO','None'):
            new_patient_data.append('No')
        elif patient_data[i] == 'YES':
            new_patient_data.append('Yes')
        else:
            new_patient_data.append('No')

    cursor.fetchall()
    cursor.close()
    # Render the template with the patient information
    return render_template('getstarted.html', patient_data=new_patient_data)
...
def doctor():
    ...
    cur.execute("""SELECT DISTINCT `PLACE` FROM `doc`""")
...
def filter_doctors():
    ...
    if place == 'All':
        ...
        cur.execute('SELECT * FROM `doc` WHERE place = %s', (place,))
    doctors = cur.fetchall()
    cur.execute('SELECT DISTINCT `PLACE` FROM `doc`')
    places = [row[0] for row in cur.fetchall()]
    cur.close()
...
def pred():
    ...
    if prediction==1:
       ...
       conn.commit()
       cursor.close()
       prediction_text="Chances of cancer is there"
...
def lungpredict():
    ...
    if prediction==1:
       ...
       name = session.get('NAME')
       print(name)
       cursor.execute("UPDATE `patients` SET `LUNG CANCER`='YES' WHERE `NAME`=%s", (name,))
       conn.commit()
       cursor.close()
       prediction_text = "Chances of cancer is there"
    ...
    if prediction==0:
        ...
        conn.commit()
        cursor.close()
        prediction_text = "Chances of cancer is high"
        return render_template("lungcancer.html", prediction_text = prediction_text)
